...
For the purposes of this mock-up analysis, we adopted a 2-component
solution top-down (in accordance with the two-senses expectation),
interpreting each component as a distinct hypothesized sense of
\textit{imagine}. To visualize separability, principal component
analysis (PCA) was applied to the standardized annotation space, and
cluster assignments were projected onto the first two principal
components. The 2-component GMM identified two clusters that were
clearly separable in the reduced two-dimensional PCA space (see Figure
\ref{fig:pcaGMM}). Visual inspection indicates no overlap between
clusters, suggesting that the three annotation dimensions jointly
support a coherent partitioning of the data into two latent sense
groupings.
...
hypothesized senses of \textit{imagine} are distributed across the joint
...
performance, with a cross-validated accuracy of 98.3\%, with optimal
...
We analyzed contextual embeddings of sentences containing the term
\textit{imagine} to investigate how different senses of the word are
represented in latent semantic space. This embedding analysis explores
the geometric structure of the representations, the relationship between
semantic similarity and sense membership, and the alignment of
embeddings with the annotated semantic dimensions. The approach provides
a comprehensive view of how contextual usage patterns are captured by
the language model (BERT) and how they relate to theoretical
distinctions between senses (note that at the moment we use inductive
sense distinctions still).
...
other sense centroid. We then estimated the kernel density (KDE)
...
clearly separated from the other other sense in centroid space than
tokens of Sense 2.
...
dimensions (intentionality, factivity, and pictoriality), we trained
simple linear regression models predicting each z-scaled dimension from
the sentence embeddings. The regression coefficients define a direction
in embedding space along which the dimension increases. By projecting
each sentence embedding onto these directions, we obtain an
embedding-based estimate for that dimension. This procedure was repeated
for all three dimensions, resulting in a 3D projection of each sentence
in the space spanned by the semantic dimensions.
...
of dimensions. The projections for factivity and intentionality (Fig.
\ref{fig:2D_IF}) shows that Sense 1 was predominantly associated with
high factivity and low intentionality, whereas Sense 2 showed the
opposite pattern. Both distributions exhibited slight bimodality,
though. In the pictoriality--intentionality projection (Fig.
\ref{fig:2D_IP}), pictoriality contributed minimally, with sense
clusters largely segregated along the intentionality axis. In contrast,
the pictoriality--factivity projection (Fig. \ref{fig:2D_FP}) revealed
...
does not reliably differentiate sense membership.
